...
export async function onRequestGet({ env }) {
  ...
  const [leads, active, content, events, pageViews, whatsapp, paypal, instagram, clients, projects] = await db.batch([
    ...
    db.prepare("SELECT COUNT(*) AS value FROM events WHERE name = 'instagram_click' AND created_at >= datetime('now','-30 day')"),
  ...
  return json({
    ...
    instagramClicks30d: instagram.results?.[0]?.value || 0,
    activeClients: clients.results?.[0]?.value || 0,
    portalProjects: projects.results?.[0]?.value || 0,
  });
}
